...
Test the export capabilities of build123d using the helper script digits.py.
...
def run_digits() -> None:
  """Run digits.py to generate export files."""

  env = os.environ.copy()
  # Avoid popping up the viewer if ocp_vscode is installed.
  env.setdefault("OCP_RENDER", "0")

  print("Running digits.py to generate missing files...")
...
def ensure_exports() -> None:
  ...
  if still_missing:
    ...
    raise FileNotFoundError(f"After digits.py these files are still missing: {missing_list}")
